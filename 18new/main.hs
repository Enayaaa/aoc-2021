...
explode z = case nextValPair z of
    Just z'@(Pair (Val a) (Val b), bs) -> if length bs >= 4
        then
            if exploded /= z'
            then Just exploded
        ...
        where Just prev' = modifyPair (const (Val 0)) z'
              left = goPrevVal prev'
              prev = case left of
...
split z@(Val n, bs)
    ...
    | otherwise = case goNextVal z of
        Just z' -> if z' == z then Just z else maybe (Just z) Just (split z')
        Nothing -> Just z
    where (v,r) = n `divMod` 2
...
reduce' z = if splitted == Just z then splitted else splitted >>= reduce'
    where
        ...
        splitted = if exploded == Just z
            then exploded >>= leftMost >>= split >>= topMost
            else exploded

reduce :: Snum -> Snum
reduce snum = fst res
    where Just res = reduce' (snum, [])

magnitude (Pair l r) = 3*magnitude l + 2*magnitude r
magnitude (Val n) = n

solve m acc [] = (acc, m)
solve m acc (s:snums) = let (res, m') = foo (acc, m) s in
    solve m' res snums--foldl foo (head snums, M.empty) (tail snums)
    where
